...
from modules.mixers.attvdn import ATTVDNMixer
...
class NQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        elif args.mixer == "attvdn":
            self.mixer = ATTVDNMixer(args)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        with th.no_grad():
            ...
            if self.args.mixer == 'attvdn':
                target_max_qvals = self.target_mixer(target_max_qvals, batch["state"], batch['actions'])
            else:
        ...
        # Mixer
        if self.args.mixer == 'attvdn':
            chosen_action_qvals = self.mixer(chosen_action_qvals_, batch["state"][:, :-1], batch['actions'][:, :-1])
        else:
